...
def m_step(data, responsibilities):
    ...
    sigma = jnp.zeros((K, D, D))
    for k in range(K):
        diff = data - mu[k]
        sigma_k = jnp.dot(responsibilities[:, k] * diff.T, diff) / jnp.sum(responsibilities[:, k])
        sigma = sigma.at[k].add(sigma_k)
...
def plot_gmm(data, pi, mu, sigma, filename='gmm_clustering.png'):
    ...
    mu: Means of the Gaussians, shape (K, D)
    sigma: Covariance matrices of the Gaussians, shape (K, D, D)
    filename: Name of the file to save the plot, with file extension (e.g., 'gmm_clustering.png')
    """
...
true_mu = jnp.array([[2.0, 3.0], [7.0, 5.0], [6.0, 10.0]])
true_sigma = jnp.array([[[1.0, 0.5], [0.5, 1.0]], [[1.5, 0.2], [0.2, 0.8]], [[1.0, -0.6], [-0.6, 1.0]]])
data = jnp.vstack([random.multivariate_normal(random.PRNGKey(k), true_mu[k], true_sigma[k], (N // K,)) for k in range(K)])
